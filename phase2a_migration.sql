...
-- 2 & 3. WORK_SESSIONS TABLE CREATION
...
-- 7. INDEXES
-- Ensure performance index for querying schedules exists
...
/*
-- Verify estimated_minutes exists on tasks (Should return 1 row with type integer)
SELECT column_name, data_type 
FROM information_schema.columns 
WHERE table_schema = 'public' AND table_name = 'tasks' AND column_name = 'estimated_minutes';

-- Verify work_sessions exists and check columns/types (Should list 9 columns)
SELECT column_name, data_type, is_nullable, column_default
FROM information_schema.columns
WHERE table_schema = 'public' AND table_name = 'work_sessions'
ORDER BY ordinal_position;

-- Verify RLS is enabled on work_sessions (relrowsecurity should be 'true' or 't')
SELECT relname, relrowsecurity 
FROM pg_class 
WHERE relname = 'work_sessions' AND relnamespace = 'public'::regnamespace;

-- Verify policy exists (Should return 'Users manage their own work sessions' for ALL commands)
SELECT policyname, permissive, roles, cmd, qual, with_check 
FROM pg_policies 
WHERE schemaname = 'public' AND tablename = 'work_sessions';

-- Verify grants exist for authenticated role (Should list SELECT, INSERT, UPDATE, DELETE)
SELECT grantee, privilege_type 
FROM information_schema.role_table_grants 
WHERE table_schema = 'public' AND table_name = 'work_sessions' AND grantee = 'authenticated';

-- Verify index exists (Should return idx_work_sessions_user_time)
