  ...
  return (
    <div className="max-w-lg mx-auto space-y-6 pb-24">
      ...
      <div className={`animate-fade-in-up stagger-2 glass-surface rounded-[22px] p-6 flex flex-col justify-between relative overflow-hidden ${
        ...
        {/* Decorative background glow for the ring */}
          ...
          <div className="relative flex items-center justify-center w-24 h-24">
            <svg className="w-full h-full transform -rotate-90" viewBox="0 0 100 100">
              ...
              <circle
                ...
                stroke="currentColor"
                strokeWidth="8"
                strokeDasharray={`${overall.percentage === null ? 0 : (overall.percentage / 100) * 264} 264`}
                strokeLinecap="round"
            ...
            <div className="absolute flex flex-col items-center justify-center">
              <span className={`text-xl font-bold tracking-tighter ${getStatusColor(overall.level)}`}>
                {Math.round(overall.percentage || 0)}<span className="text-xs">%</span>
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="mt-6 flex justify-between border-t border-white/[0.06] pt-4 z-10">
          <div className="flex-1">
            <p className="text-xl font-semibold text-emerald-400 tabular-nums drop-shadow-sm">{overall.totalPresent}</p>
          ...
          <div className="flex-1 border-l border-white/[0.06] pl-4">
            <p className="text-xl font-semibold text-red-400 tabular-nums drop-shadow-sm">{overall.totalAbsent}</p>
            <p className="text-[10px] text-white/50 uppercase tracking-widest font-medium mt-0.5">Absent</p>
          </div>
          <div className="flex-1 border-l border-white/[0.06] pl-4">
            <p className="text-xl font-semibold text-zinc-200 tabular-nums drop-shadow-sm">{overall.totalConducted}</p>
          ...
          <div className="space-y-3">
            {todaySessions.map((session) => {
              ...
              return (
                <div
                  key={session.id}
                  className="glass-elevated flex items-center justify-between rounded-[18px] px-4 py-3"
                >
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[10px] rounded-full bg-white/10 border border-white/5 px-2 py-0.5 font-medium text-zinc-300">
                        {session.classType}
                      </span>
                    </div>
      ...
      <div className="animate-fade-in-up stagger-4">
        ...
        <div className="space-y-3">
          {subjectStats.map(({ subject, attendance }, idx) => (
            <div key={subject.code} className={`animate-fade-in-up stagger-${Math.min(idx + 1, 5)}`}>
